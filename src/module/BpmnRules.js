// Customization Begin
/* eslint-disable */
// connection.updateWaypoints, canConnect are customized below.
...
BpmnRules.prototype.init = function() {
  ...
  this.addRule('connection.create', function(context) {
    var source = context.source;
        ...
        var targetParent = hints.targetParent;
        var targetAttach = hints.targetAttach;
...
function canConnect(source, target, connection) {
  // connections checking helper
  function checkConnections(source, target){
    if (
      source &&
      source.outgoing &&
      source.outgoing.find(item => item.target === target) !== undefined
    ) {
      return true
    }
    return false
  }
  // disable loop connections, mutiple connections, reverse connections
  if(source === target|| checkConnections(source, target) || checkConnections(target, source)) {
    return false
  }
...
function isOneTextAnnotation(source, target) {
  ...
  var sourceTextAnnotation = isTextAnnotation(source);
      var targetTextAnnotation = isTextAnnotation(target);
